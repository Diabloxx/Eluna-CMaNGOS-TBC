...
//Custom Code
bool ChatHandler::HandleXPCommandDisable(char* /*args*/)
{
    Player* player = m_session->GetPlayer();

    if (player->IsXPDisabled())
    {
        PSendSysMessage("XP gain is already disabled.");
    }
    else
    {
        player->DisableXPGain();
        PSendSysMessage("XP gain has been disabled.");
    }

    return true;
}

bool ChatHandler::HandleXPCommandEnable(char* /*args*/)
{
    Player* player = m_session->GetPlayer();
    ...
    {
        player->EnableXPGain();
        PSendSysMessage("XP gain has been enabled.");
    }
    else
    {
        PSendSysMessage("XP gain is already enabled.");
    }
